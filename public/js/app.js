...
window.openModal = function (id) {
    ...
    if (id === "modalTambahPegawai") {
        window.tambahPage = 1;
        if (typeof window.renderTambah === "function") window.renderTambah();
    }
...
window.toggleDropdown = function (id, btn) {
    ...
    const isCollapsed = sidebar && sidebar.dataset.expanded === "false";
...
window.toggleSidebar = function () {
    ...
    if (isExpanded) {
        ...
        document.querySelectorAll("#sidebar .nav-icon").forEach(function (el) {
            el.style.margin = "0";
        });

        const logoCircle = sidebar.querySelector(".sidebar-logo");
        if (logoCircle) logoCircle.style.justifyContent = "center";

        const sidebarUser = sidebar.querySelector(".sidebar-user");
        if (sidebarUser) sidebarUser.style.justifyContent = "center";
        ...
        if (footer)  footer.style.marginLeft    = "210px";
        ...
        const logoCircle = sidebar.querySelector(".sidebar-logo");
        if (logoCircle) logoCircle.style.justifyContent = "";

        const sidebarUser = sidebar.querySelector(".sidebar-user");
        if (sidebarUser) sidebarUser.style.justifyContent = "";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (jamEl) {
        function tick() { jamEl.textContent = new Date().toLocaleTimeString("id-ID"); }
...
window.tambahPage = 1;
window.editPage   = 1;
window.tambahNext = function () { window.tambahPage = 2; window.renderTambah(); };
window.tambahPrev = function () { window.tambahPage = 1; window.renderTambah(); };
window.editNext   = function () { window.editPage   = 2; window.renderEdit();   };
window.editPrev   = function () { window.editPage   = 1; window.renderEdit();   };
...
window.renderTambah = function () {
    ...
    if (page1)  { page1.classList.toggle("hidden", window.tambahPage !== 1); page1.style.display = window.tambahPage === 1 ? "grid" : "none"; }
    if (page2)  { page2.classList.toggle("hidden", window.tambahPage !== 2); page2.style.display = window.tambahPage === 2 ? "grid" : "none"; }
    if (title)  title.textContent   = window.tambahPage === 1 ? "Data Pribadi" : "Data Kepegawaian";
    if (sub)    sub.textContent     = "Halaman " + window.tambahPage + " / 2";
    if (btnPrev)  { btnPrev.classList.toggle("hidden", window.tambahPage === 1);  btnPrev.style.display  = window.tambahPage === 1 ? "none" : ""; }
    if (btnNext)  { btnNext.classList.toggle("hidden", window.tambahPage === 2);  btnNext.style.display  = window.tambahPage === 2 ? "none" : "flex"; }
    if (btnSimpan){ btnSimpan.classList.toggle("hidden", window.tambahPage !== 2); btnSimpan.style.display= window.tambahPage === 2 ? "flex" : "none"; }
};
...
window.renderEdit = function () {
    ...
    if (page1)  { page1.classList.toggle("hidden", window.editPage !== 1); page1.style.display = window.editPage === 1 ? "grid" : "none"; }
    if (page2)  { page2.classList.toggle("hidden", window.editPage !== 2); page2.style.display = window.editPage === 2 ? "grid" : "none"; }
    if (title)  title.textContent   = window.editPage === 1 ? "Edit Data Pribadi" : "Edit Data Kepegawaian";
    if (sub)    sub.textContent     = "Halaman " + window.editPage + " / 2";
    if (btnPrev)  { btnPrev.classList.toggle("hidden", window.editPage === 1);  btnPrev.style.display  = window.editPage === 1 ? "none" : ""; }
    if (btnNext)  { btnNext.classList.toggle("hidden", window.editPage === 2);  btnNext.style.display  = window.editPage === 2 ? "none" : "flex"; }
    if (btnSimpan){ btnSimpan.classList.toggle("hidden", window.editPage !== 2); btnSimpan.style.display= window.editPage === 2 ? "flex" : "none"; }
};
...
function initDataTable(table) {
    ...
    var page = 1, sortCol = -1, sortAsc = true;

    allRows.forEach(function (tr) {
        tr.dataset.cells = Array.from(tr.querySelectorAll("td"))
            .map(function (td) { return td.textContent.trim().toLowerCase(); })
            .join("|");
    });
    ...
    footer.style.cssText = "display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.5rem;padding:0.75rem 1rem;border-top:1px solid #30363d;background:#161b22;border-radius:0 0 10px 10px;";
    footer.innerHTML =
    ...
    function doFilter(q) {
        ...
        filtered = q ? allRows.filter(function (tr) { return tr.dataset.cells.includes(q); }) : allRows.slice();
        doSort();
    }
    function doSort() {
        if (sortCol < 0) return;
        ...
        filtered.sort(function (a, b) {
            var tA = ((a.querySelectorAll("td")[sortCol] || {}).textContent || "").trim();
            var tB = ((b.querySelectorAll("td")[sortCol] || {}).textContent || "").trim();
    ...
    function render() {
        ...
        var start = (page - 1) * PER_PAGE, end = Math.min(start + PER_PAGE, total);
        tbody.innerHTML = "";
        if (total === 0) {
            tbody.innerHTML = '<tr><td colspan="' + headers.length + '" style="text-align:center;color:#8b949e;font-size:0.875rem;padding:2rem;">Tidak ada data ditemukan.</td></tr>';
        } else {
            filtered.slice(start, end).forEach(function (tr) { tbody.appendChild(tr); });
        }
        infoEl.style.color = "#ffffff";
        infoEl.textContent = total === 0 ? "0 data" : "Menampilkan " + (start + 1) + "–" + end + " dari " + total + " data";
        ...
        headers.forEach(function (th, idx) {
            ...
            if (idx === sortCol && th.dataset.sortable !== "false") {
                ...
                i.className = "dt-sort-icon bi bi-caret-" + (sortAsc ? "up" : "down") + "-fill";
                i.style.cssText = "margin-left:4px;font-size:10px;color:#58a6ff;";
                th.appendChild(i);
            }
        });
    }
    function renderPagination(totalPages) {
        var btnStyle = "display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;padding:0 6px;font-size:0.75rem;border-radius:6px;border:1px solid #30363d;background:#21262d;color:#c9d1d9;cursor:pointer;transition:background 0.15s;";
        var html = "";
        html += '<button style="' + btnStyle + '" data-page="' + (page - 1) + '" ' + (page === 1 ? "disabled" : "") + '><i class="bi bi-chevron-left" style="font-size:10px"></i></button>';
        pageRange(page, totalPages).forEach(function (p) {
            if (p === "...") {
                html += '<span style="padding:0 4px;font-size:0.75rem;color:#8b949e;">…</span>';
            } else {
                var active = p === page ? "background:#238636;border-color:#238636;color:#fff;font-weight:600;" : "";
                html += '<button style="' + btnStyle + active + '" data-page="' + p + '">' + p + "</button>";
            }
        });
        html += '<button style="' + btnStyle + '" data-page="' + (page + 1) + '" ' + (page === totalPages ? "disabled" : "") + '><i class="bi bi-chevron-right" style="font-size:10px"></i></button>';
        pagEl.innerHTML = html;
        pagEl.querySelectorAll("button[data-page]:not([disabled])").forEach(function (btn) {
...
window.showDetailPegawai = function (data) {
    var ttl   = (data.viewTempatLahir || "-") + ", " + (data.viewTanggalLahir || "-");
    var ttlEl = document.getElementById("viewTTL");
    if (ttlEl) ttlEl.textContent = ttl;

    var map = {
        viewNIK: "viewNIK", viewNIP: "viewNIP", viewNama: "viewNamaTitle",
        viewJenisKelamin: "viewJenisKelamin", viewAgama: "viewAgama",
        viewStatusPernikahan: "viewStatusNikah", viewNoTelepon: "viewTelp",
        viewAlamat: "viewAlamat", viewJabatan: "viewJabatan",
        viewDepartemen: "viewDepartemen", viewGolongan: "viewGolongan",
        viewPendidikan: "viewPendidikan", viewTglMasuk: "viewTglMasuk",
        viewJenisPegawai: "viewJenisPegawai",
    };
    Object.entries(data).forEach(function (entry) {
        if (entry[0] === "viewTempatLahir" || entry[0] === "viewTanggalLahir") return;
        var elId = map[entry[0]];
        if (elId) { var el = document.getElementById(elId); if (el) el.textContent = entry[1] || "-"; }
    });
    ...
    if (status) status.innerHTML = '<span class="' + data.viewStatusClass + ' mt-1 inline-block">' + data.viewStatus + "</span>";

    var dept = document.getElementById("viewDepartemen");
    if (dept) dept.innerHTML = '<span class="status-departemen mt-1 inline-block">' + data.viewDepartemen + "</span>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (loginData) {
        ...
        var errorMsg    = loginData.dataset.error;
        ...
        if (redirectUrl && redirectUrl.trim() !== "") {
            ...
            var timer = setInterval(function () {
                ...
                if (sisa <= 0) { clearInterval(timer); window.location.href = redirectUrl; }
            }, 1000);
        }

        if (errorMsg && errorMsg.trim() !== "") {
            var errModal = document.getElementById("modalLoginError");
            if (errModal) { errModal.classList.remove("hidden"); errModal.style.display = "flex"; }
        }
    }
...
window.handlePhotoPreview = function (event, previewContainerId, previewImgId) {
    ...
    if (!file) { if (container) container.style.display = "none"; return; }
    ...
    reader.onload = function (e) {
        if (img) { img.src = e.target.result; if (container) container.style.display = "block"; }
    };
