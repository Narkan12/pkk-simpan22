...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <?php if(session('success')): ?>
        tampilkanToast(<?php echo json_encode(session('success'), 15, 512) ?>, 'sukses');
    <?php endif; ?>

    
    <?php if(session('error')): ?>
        tampilkanToast(<?php echo json_encode(session('error'), 15, 512) ?>, 'error');
    <?php endif; ?>

    
    <?php if(session('warning')): ?>
        tampilkanToast(<?php echo json_encode(session('warning'), 15, 512) ?>, 'warning');
    <?php endif; ?>

    
    <?php if(session('danger')): ?>
        tampilkanToast(<?php echo json_encode(session('danger'), 15, 512) ?>, 'warning');
    <?php endif; ?>

    
    <?php if($errors->any()): ?>
        const daftarError = <?php echo json_encode($errors->all(), 15, 512) ?>;
...
<?php /**PATH C:\laragon\www\PKK-Simpan22\resources\views\components\notifikasi-toast.blade.php ENDPATH**/ ?>