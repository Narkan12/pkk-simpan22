...
class ManajemenController extends Controller
{
    ...
    public function absensi(Request $request)
    {
        ...
        // alias biar nama variabel di view tetap sama
    ...
    public function cutiSetujui(Request $request, Cuti $cuti)
    {
        $request->validate([
        ...
        // format: "hrd01 (HRD)"
        $user = Auth::user();
        $diputuskanOleh = $user->username . ' (' . ucfirst($user->role) . ')';
    ...
    public function cutiTolak(Request $request, Cuti $cuti)
    {
        $request->validate([
            'catatan_keputusan' => 'nullable|string|max:500',
        ]);
    ...
    {
        // ambil data cuti + relasi yang dibutuhkan
        $cuti = Cuti::with(['pegawai.jabatan', 'pegawai.departemen'])->findOrFail($id);

        // belum bisa dicetak kalau masih pending
        if ($cuti->status === 'pending') {
            abort(403, 'Surat belum dapat dicetak karena pengajuan cuti masih menunggu keputusan.');
        }

        // nomor surat dibuat sekali, kalau sudah ada pakai yang lama
        if (is_null($cuti->nomor_surat)) {
            $nomorSurat = 'SK-CUTI/' . $cuti->tanggal_mulai->format('Y') . '/' . str_pad($cuti->id, 5, '0', STR_PAD_LEFT);
            $cuti->update(['nomor_surat' => $nomorSurat]);
        }

        return view('cuti.surat-cetak', compact('cuti'));
    }
