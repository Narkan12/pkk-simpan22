...
@section('content')
    ...
    @endphp
    <div style="margin-top:1rem;border-radius:0.5rem;overflow:hidden;border:1px solid {{ $warna['border'] }};background:{{ $warna['bg'] }};">

        {{-- Baris utama notifikasi --}}
        <div style="padding:0.75rem 1rem;display:flex;align-items:center;gap:0.75rem;">
            ...
            {{-- Tombol cetak surat — hanya muncul jika sudah ada keputusan --}}
            @if($c->status !== 'pending')
            <a href="{{ route('manajemen-cuti.cetak', $c->id) }}" target="_blank"
                style="flex-shrink:0;display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;font-size:0.75rem;font-weight:600;text-decoration:none;background:{{ $warna['dot'] }};color:#fff;">
                <i class="bi bi-printer"></i> Cetak
            </a>
            @endif
        </div>

        {{-- Catatan keputusan dari pimpinan — hanya tampil jika ada --}}
        @if($c->status !== 'pending' && $c->diputuskan_oleh)
        <div style="padding:0.5rem 1rem 0.75rem;border-top:1px solid {{ $warna['border'] }};">
            <p style="font-size:0.7rem;color:#7a9a7a;margin:0 0 2px;">
                <i class="bi bi-person-badge-fill me-1" style="color:{{ $warna['dot'] }};"></i>
                Keputusan oleh <strong style="color:#1a2e1a;">{{ $c->diputuskan_oleh }}</strong>
                @if($c->diputuskan_pada)
                    &mdash; {{ \Carbon\Carbon::parse($c->diputuskan_pada)->isoFormat('D MMM Y, HH:mm') }}
                @endif
            </p>
            @if($c->catatan_keputusan)
            <p style="font-size:0.8rem;color:#1a2e1a;font-style:italic;margin:2px 0 0;">
                "{{ $c->catatan_keputusan }}"
            </p>
            @endif
        </div>
